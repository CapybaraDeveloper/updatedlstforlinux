    ...
    if [ "$kernel" = "Linux" ]; then
        ...
        find_package_manager() {
            case "$1" in
                ...
                fedora|almalinux|rocky|rhel|centos|oracle|redos) echo "if command -v rpm-ostree >/dev/null 2>&1; then $SUDO rpm-ostree upgrade -y && $SUDO rpm-ostree install -y git; else $SUDO yum makecache -y && $SUDO yum install -y git; fi" ;;
